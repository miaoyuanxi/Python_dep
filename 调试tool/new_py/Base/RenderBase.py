...
class RenderBase(object):
    ...
    def __init__(self, **param_dict):
        ...
        common = self.G_TASK_JSON_DICT['system_info']['common']
        ...
        if 'scene_info_render' in self.G_TASK_JSON_DICT:
            render_common = self.G_TASK_JSON_DICT['scene_info_render']['common']
            self.G_TASK_FRAMES = render_common['frames']  # 任务需要渲染的帧
        self.render_record = {}
        # 完成队列
        ...
        if os.path.exists(dir):
            ...
            for root, dirs, files in list_dirs:
                for name in files:
                    ...
                    if ext in ['.vrmap', '.vrlmap', '.exr', '.vrimg']:
                        continue
                    work_big_pic = os.path.join(root, name)
                    ...
                    if ext == '.exr' and os.path.exists(oiio_path):
                        ...
                        convert_cmd = oiio_path + ' "' + work_big_pic_tmp + '" -resize ' + self.get_convert_r(
                            work_big_pic_tmp, oiio_path) + ' -o "' + work_small_tmp_pic + '"'
                    # print work_big_pic
                    try:
    ...
    def rendering_copy_notify(self, start_or_end='0'):
        ...
        try:
            ...
            # self.G_DEBUG_LOG.info("[response] {}".format(response.read()))
            print("模拟发请求 通知平台")
            time.sleep(1)
        except urllib.error.HTTPError as e:
            self.G_DEBUG_LOG.info('[err]url can\'t open: {}'.format(rendering_copy_interface))
            success = False
        return success
    ...
    def save_render_info(self, frame):
        ...
        if os.path.isfile(path):
            ...
            frame_list = [frame]
            task_all = common_util.need_render_from_frame(self.G_TASK_FRAMES)
            obj = {
                "task_all": task_all,  # ['1', '3', '5', '7', '9']
    ...
    def write_deduct_fee_text(self, frame, big_pic_str, small_pic_str):
        ...
        try:
            ...
            print("[write_deduct_fee_text] error: {}, frame: {}".format(e, frame))
            return False
        return True

    def copy_small_pic(self):
        print("copy small picture: {}->{}".format(self.G_WORK_RENDER_TASK_SMALL, self.G_SMALL_PATH))
        cmd = r'c:\fcopy\FastCopy.exe /cmd=force_copy /speed=full /force_close /no_confirm_stop /force_start "{}" /to="{}"'.format(
    ...
    def remain_frames(self):
        ...
        if os.path.isfile(json_path) and int(self.G_RECOMMIT_FLAG) > 0:
