...
class RenderMax(Max):
    ...
    def RB_RENDER(self):  # 5
        ...
        if self.g_one_machine_multiframe is True:
            if int(self.G_RECOMMIT_FLAG) > 0:
                cmd = r'c:\fcopy\FastCopy.exe /cmd=force_copy /speed=full /force_close /no_confirm_stop /force_start "{}" /to="{}"'.format(
                    os.path.join(self.G_REMOTE_TASK_CFG, os.path.basename(self.render_record_json)),
        ...
        # ------------wtite ms file----------
            task_ms_file = self.write_ms_file1(frames)
        else:
    ...
    def write_ms_file1(self, frames):
        not_render = 'false'
        if self.G_CG_VERSION == '3ds Max 2012' or self.G_CG_VERSION == '3ds Max 2011' or self.G_CG_VERSION == '3ds Max 2010' or self.G_CG_VERSION == '3ds Max 2009':
            render_ms_name = 'rendera2.0.ms'
            json_file = self.G_TASK_JSON_A
        ...
        filein @"{render_ms}"
        fn renderRun = (
            web_render #("{user_id}","{task_id}","not_render","{frames}","{action_id}","{option}","{action}","{task_output_xxx}/","{max_file}", @"{json_file}", "{kg}")
        )
        '''.format(
            render_ms=render_ms,
    ...
    def get_render_frame(self):
        ...
        self.G_DEBUG_LOG.info("[get_render_frame]self.G_CG_FRAMES 1: {}\nself.G_KG: {}\nself.G_ACTION: {}".format(self.G_CG_FRAMES, self.G_KG, self.G_ACTION))
        self.G_DEBUG_LOG.info("[get_render_frame] log dict {}".format(self.G_TASK_JSON_DICT['scene_info_render']))

        if self.G_KG == '100':  # inc
            if self.G_ACTION == 'RenderPhoton':
                if 'gi_frames' in self.G_TASK_JSON_DICT['scene_info_render']['renderer']:
                    render_frame = self.G_TASK_JSON_DICT['scene_info_render']['renderer']['gi_frames']
                    self.G_DEBUG_LOG.info("1111111111 {}".format(render_frame))
                else:
        ...
        # if light_cache_frame!=None:
        # render_frame=light_cache_frame
        self.G_DEBUG_LOG.info("[get_render_frame]self.G_CG_FRAMES 2: {}\nself.G_KG: {}\nself.G_ACTION: {}".format(self.G_CG_FRAMES, self.G_KG, self.G_ACTION))
