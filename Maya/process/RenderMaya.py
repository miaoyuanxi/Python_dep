...
class RenderMaya(Maya):
	...
	def RB_RENDER(self):  # 5
		...
		if int(self.G_CG_TILE_COUNT) != int(self.G_CG_TILE):
			...
			self.G_DEBUG_LOG.info(
				"\n\n-------------------------------------------Start maya program-------------------------------------\n\n")
			result_code, _ = CLASS_COMMON_UTIL.cmd(render_cmd, my_log=self.G_RENDER_LOG, continue_on_error=True,
												   my_shell=True,
	...
	def get_render_cmd(self):
		...
		cmd += " -preRender \"python \\\"pre_render_dict=%s;execfile(\\\\\\\"%s\\\\\\\")\\\"\"" % (
		pre_render_dict, self.G_RN_MAYA_PRERENDER)
