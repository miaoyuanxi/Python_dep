...
class PreMaya(Maya):
    def __init__(self,**paramDict):
        ...
        self.format_log('PreMaya.init','start')
...
class MayaEnv():
    def __init__(self,pluginCfg,myLog=None):
        ...
        if self.G_PLUGIN_DICT and self.G_PLUGIN_DICT.has_key('plugins'):
            pluginDict=self.G_PLUGIN_DICT['plugins']
            pluginDict['rayvision']='1.0.0'
        if self.G_PLUGIN_DICT.has_key('renderSoftware') or self.G_PLUGIN_DICT.has_key('softwareVer'):
        ...
        if allPlugDict and allPlugDict.has_key('plugins'):
            ...
            for pluginName in pluginDict:
                if pluginName=="vray":
                    ...
                    unpackCmd='c:/7-Zip/7z.exe x "'+d7z+'" -y -aoa -o"c:/Program Files" '
