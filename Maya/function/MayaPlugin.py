...
class PluginBase(object):
    def __init__(self):
        ...
        if self.CURRENT_OS == "linux":
            self.JSON_PATH = r'/tmp/nzs-data/renderbuswork/cg/maya/linux/plugins/envInfo.json'
            self.ZIP_TYPE = ".zip"
        ...
        if allPlugDict and 'plugins' in allPlugDict:
            ...
            if pluginDict:
                for pluginName in pluginDict:
                        ...
                        if pluginName == 'miarmy':
                            ...
                            for zipfile in os.listdir(ZipSourceDir):
                                ...
                                if zipfile.startswith(pluginName) and zipfile.endswith(self.ZIP_TYPE):
                                    ver_list.append(
                                        int(os.path.splitext(zipfile)[0].split(pluginName)[1].replace(".", "")))
                            if len(ver_list) == 0:
                    ...
                    pluginPyPath01 = os.path.join(self.MAYA_Plugin_Dir, pluginName, 'script',
                                                  self.G_PLUGIN_PY_NAME).replace('\\', '/')
                    pluginPyPath02 = os.path.join(self.MAYA_Plugin_Dir, pluginName, 'script',
                                                  self.G_CG_NAME + self.G_CG_VERSION + '_' + pluginName + pluginVersion,
                                                  self.G_PLUGIN_PY_NAME).replace('\\', '/')
                    pluginZipSource = os.path.join(self.MAYA_Plugin_Dir, pluginName, 'source', pluginZipName).replace(
                        '\\', '/')
                    pluginZipSource_pro = os.path.join(self.MAYA_Plugin_Dir, pluginName, 'source',
                                                       pluginZipName_pro).replace('\\', '/')
                    pluginZipSource_clientM = os.path.join(self.MAYA_Plugin_Dir, pluginName, 'source',
                                                           pluginZipName_clientM).replace('\\', '/')
                    ...
                    if os.path.exists(pluginConfigFile):
                        if os.path.exists(pluginZipSource) or os.path.exists(pluginZipSource_pro) or os.path.exists(
                                pluginZipSource_clientM):
                            ...
                            index += 1
                            self.MyLog(
                                '[Error]----The  plugin %s version  %s  is not exists!' % (pluginName, pluginVersion))
                    else:
        ...
        if len(pluginValidList) > 0:
            for plugindict in pluginValidList:
                if plugindict:
                    ...
                    PluginDir = os.path.join(self.Node_D, plugindict['pluginName'], 'software',
                                             self.G_CG_NAME + self.G_CG_VERSION + '_' + plugindict['pluginName'] +
                                             plugindict['pluginVersion']).replace('\\', '/')
