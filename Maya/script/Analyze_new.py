...
class Analyze(dict,Maya):
    ...
    def check_default_layer_name(self):
        renderLayers = pm.ls(exactType="renderLayer")
        referenceLayers = pm.ls(exactType="renderLayer", rn=1)
        layer_cont = len(renderLayers) - len(referenceLayers)
        if layer_cont == 1:
            if renderLayers != "defaultRenderLayer":
                self.writing_error(25019,renderLayers)

        else:
            if "defaultRenderLayer" not in renderLayers:
                self.writing_error(25019, renderLayers)
        ...
        for node_i in nodes:
            ...
            if overrides:
                for layer in self.get_all_render_layers():
                    if self.get_node_attr_over(node_i, "useFrameExtension", layer) or self.get_node_attr_over(node_i, "uvTilingMode", layer):
                        amin_attr = 1
                ...
                if self.get_node_attr_val(node_i, "useFrameExtension") or self.get_node_attr_val(node_i, "uvTilingMode"):
                    amin_attr = 1
            ...
            for i in self.asset_dict:
                for j in self.asset_dict[i]:
                    if len(self.asset_dict[i][j]["local_path"]) >0:
                        file_list.append({j:self.asset_dict[i][j]["local_path"]})
                    if len(self.asset_dict[i][j]["missing"]) >0:
                        miss_files.append({j:self.asset_dict[i][j]["missing"]})
                    for env_i in self.asset_dict[i][j]["env"]:
                        if env_i not in env_dict:
                            env_dict[env_i] = self.asset_dict[i][j]["env"][env_i]
        self.asset_info_dict['maya_file'] = [self.str_to_unicode(self["cg_file"])]
        self.asset_info_dict["file_list"] = file_list
        self.asset_info_dict["miss_files"] = miss_files
        self.asset_info_dict["env"] = env_dict
