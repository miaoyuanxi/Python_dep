...
class PreRender(dict,PreRenderBase):
    ...
    def han_custome_prerender_bak(self):
        ...
        if self.user_id not in [1813119]:
            if self.rendersetting:
                if 'renderableLayer' in self.rendersetting:
                    if "," in self.rendersetting['renderableLayer']:
                        self.set_layer_render(self.rendersetting['renderableLayer'])
        if self.user_id in [1865164, 1163153]:
            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")

        if self.user_id in [1874138]:
        ...
        if self.user_id in [1841631, 1870881]:
            if self.rendersetting:
                if 'renderableLayer' in self.rendersetting:
                    if "," in self.rendersetting['renderableLayer']:
                        self.set_layer_render(self.rendersetting['renderableLayer'])
        ...
        if self.user_id in [1868564]:
            ...
            print self.start
            print self.plugins
            print "test   prerender  success!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
            sys.stdout.flush()

        if self.user_id in [1850226, 1852174, 1854599]:
            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")
            print self.start
        ...
        if self.user_id in [1848517]:
            print self.start
            if self.start:
                cmds.currentTime(int(self.start - 1), edit=True)
                print "update to pre frame"

        if self.user_id in [1860293]:
            # change_path_self.mapping("AlembicNode", "abc_File",self.mapping)
            print "the mode++++++"
            print cmds.evaluationManager(query=True, mode=1)
            print "the mode++++++"
            cmds.refresh()
        if self.user_id in [1879929]:
            print "the user id is 1879929"
            # change_path_self.mapping("AlembicNode", "abc_File",self.mapping)
            # cmds.XgPreview()
            cmds.refresh()

        if self.user_id in [1865216]:
            for i in pm.ls(type="aiOptions"):
        ...
        if self.user_id in [1807271]:
            self.change_path_mapping("ffxDyna", "output_path", self.mapping)
            self.change_path_mapping("ffxDyna", "wavelet_output_path", self.mapping)
            self.change_path_mapping("ffxDyna", "postprocess_output_path", self.mapping)

        if self.user_id in [1852657]:
            mel.eval(
                'source "C:/Program Files/Domemaster3D/maya/common/scripts/domeRender.mel"; domemaster3DPreRenderMEL();')
        ...
        if self.user_id in [1860670]:
            curent_time = int(self.start)
            ...
            self.mapping["//THE-SERVER/Guest/Sony/GAB/"] = "J:/"
            self.mapping["//the-server/Guest/Sony/GAB/"] = "J:/"
            # attr_list.append({})
            rep_path = ""
            search = ""
        ...
        if self.user_id in [1840038, 1861572, 1867734]:
            for i in pm.ls(type="aiOptions"):
        ...
        if self.user_id in [963250, 1816742]:
            maya_proj_path = cmds.workspace(q=True, fullName=True)
            source_path = "%s/sourceimages" % (maya_proj_path)
            for i in pm.ls(type="aiOptions"):
                if i.hasAttr("log_verbosity"):
                ...
                if i.hasAttr("shader_searchpath"):
                    i.shader_searchpath.set(
                        r"B:\clientFiles\963250\htoa-2.0.0_r240c4dd_houdini-16.0.557\htoa-2.0.0_r240c4dd_houdini-16.0.557\arnold\self.plugins")
                    print 'setAttr "defaultArnoldRenderOptions.shader_searchpath:B:\clientFiles\963250\htoa-2.0.0_r240c4dd_houdini-16.0.557\htoa-2.0.0_r240c4dd_houdini-16.0.557\arnold\self.plugins'
                if i.hasAttr("plugin_searchpath"):
                    i.plugin_searchpath.set(
                        r"B:\clientFiles\963250\htoa-2.0.0_r240c4dd_houdini-16.0.557\htoa-2.0.0_r240c4dd_houdini-16.0.557\arnold\self.plugins;B:\clientFiles\963250\htoa-2.0.0_r240c4dd_houdini-16.0.557\htoa-2.0.0_r240c4dd_houdini-16.0.557\arnold\drivers")
                    print 'setAttr "defaultArnoldRenderOptions.plugin_searchpath:B:\clientFiles\963250\htoa-2.0.0_r240c4dd_houdini-16.0.557\htoa-2.0.0_r240c4dd_houdini-16.0.557\arnold\self.plugins;B:\clientFiles\963250\htoa-2.0.0_r240c4dd_houdini-16.0.557\htoa-2.0.0_r240c4dd_houdini-16.0.557\arnold\drivers '
                i.texture_searchpath.set(source_path)
                # setAttr -type "string" defaultArnoldRenderOptions.procedural_searchpath ;

            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")

        if self.user_id in [1860393, 1844206]:
            maya_proj_path = cmds.workspace(q=True, fullName=True)
            source_path = "%s/sourceimages" % (maya_proj_path)
            for i in pm.ls(type="aiOptions"):
                if i.hasAttr("log_verbosity"):
        ...
        if self.user_id in [1861025, 1858235]:
            for i in pm.ls(type="aiOptions"):
                print "defaultArnoldRenderOptions.abortOnError 0"
                i.abortOnError.set(0)
                i.log_verbosity.set(1)
        ...
        if self.user_id in [1861989]:
            ...
            if self.start:
                if int(self.start) > 3:
                    for i in range(int(self.start) - 3, int(self.start)):
        ...
        if self.user_id in [1840793, 1869176]:
            for i in pm.ls(type="VRaySettingsNode"):
                if self.rendersetting:
                    if 'width' in self.rendersetting and i.hasAttr("width"):
                        i.width.set(self.rendersetting['width'])
        ...
        if self.user_id in [963452, 1854599]:
            for i in pm.ls(type="aiOptions"):
                print "defaultArnoldRenderOptions.abortOnError 0"
                i.abortOnError.set(0)
                print "abortOnError off"

                # if self.user_id in [1821228]:
                # for i in pm.ls(type="pgYetiMaya"):
                # print "%s .aiLoadAtInit 1" % (i)
                # i.aiLoadAtInit.set(1)
        if self.user_id in [1844577, 1857698, 1853469, 1867132, 1821228]:
            for i in pm.ls(type="pgYetiMaya"):
                print "%s .aiLoadAtInit 1" % (i)
                i.aiLoadAtInit.set(1)
        if self.user_id in [1879932]:
            for i in pm.ls(type="aiOptions"):
                print "defaultArnoldRenderOptions.abortOnError 0"
                i.abortOnError.set(0)
                i.log_verbosity.set(0)
                print "set log_verbosity to 0"

        if self.user_id in [1898153]:
            for i in pm.ls(type="aiOptions"):
                print "defaultArnoldRenderOptions.abortOnError 0"
                i.abortOnError.set(0)
                i.log_verbosity.set(1)
        ...
        if self.user_id in [1880735]:
            print "set vray animType "
            for i in pm.ls(type="VRaySettingsNode"):
                if i.hasAttr("animType"):
                    i.animType.set(1)
                    print "set animType 1"
        ...
        if self.user_id in [962539]:
            ...
            for i in pm.ls(type="aiOptions"):
                ...
                i.absoluteTexturePaths.set(0)
                i.absoluteProceduralPaths.set(0)
                source_path = "%s/sourceimages" % (self.maya_proj_paht)
                i.shader_searchpath.set(source_path)
                i.texture_searchpath.set(source_path)

        if self.user_id in [1861628]:
            print "replace path "
            rep_path = ""
            search = ""
            dicts = {}
        ...
        if self.user_id in [961743, 1844765]:
            print "  "
        ...
        if self.user_id in [1866841]:
            print "set vray srdml "
            for i in pm.ls(type="VRaySettingsNode"):
                if i.hasAttr("srdml"):
                    i.srdml.set(0)

        if self.user_id in [1815272, 1861877]:
            # for i in pm.ls(type="aiOptions"):
            # i.abortOnError.set(0)
            # print 'abortOnError-off'
            for i in pm.ls(type="aiOptions"):
                i.abortOnError.set(0)
                print 'abortOnError-off'
        if self.user_id in [1837976, 1862577]:
            for i in pm.ls(type="aiOptions"):
                i.abortOnError.set(0)
                print 'abortOnError-off'
                # print self.start
        ...
        if self.user_id in [1821226]:
            self.del_node('CH_GTQScale0o032_seq000:Face_cameraShape')
            for i in pm.ls(type="pgYetiMaya"):
                # print "%s .aiLoadAtInit 1" % (i)
                i.aiLoadAtInit.set(1)
            for i in pm.ls(type="aiOptions"):
        ...
        if self.user_id in [1848903, 1863429]:
            rep_path = ""
            search = ""
            dicts = {}
            if search:
        ...
        if self.user_id in [1859072, 1859047, 1815076]:
            print "set vray srdml "
            for i in pm.ls(type="VRaySettingsNode"):
                if i.hasAttr("srdml"):
                    i.srdml.set(0)
        ...
        if self.user_id in [962276]:
            if self.task_id in [10279321]:
                for i in pm.ls(type="aiOptions"):
                    if i.hasAttr("autotx"):
                        i.autotx.set(False)
                    if i.hasAttr("use_existing_tiled_textures"):
            ...
            for i in pm.ls(type="VRaySettingsNode"):
                if i.hasAttr("srdml"):
                    ...
                    if "vrayformaya" in self.plugins:
                        print type(self.plugins['vrayformaya'])
                        if self.plugins['vrayformaya'] == "HQ" or self.plugins['vrayformaya'] >= "3.10.01":
        ...
        if self.user_id in [1840132]:
            if self.render_name == "arnold":
                ...
                print "currentRenderer is : %s" % self.render_name
            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")

        if self.user_id in [1896034]:
            if self.render_name == "arnold":
                for i in pm.ls(type="aiOptions"):
                    i.abortOnError.set(0)
                    print 'abortOnError-off'
        ...
        if self.user_id in [1863854, 1828930]:
            for i in pm.ls(type="aiOptions"):
                i.log_verbosity.set(1)
                if i.hasAttr("autotx"):
                    i.autotx.set(False)
        ...
        if self.user_id in [1855708, 900045, 1855110, 1833038, 1834849, 1857988, 1826800, 1860992, 1163411, 1862577,
                            1830380,
                            1822281, 1852326, 1868563, 1849290, 963140, 1862389, 1865352, 1864255, 1870881, 1879938,
                            1833958,
                            1895687, 1869176, 1894815, 1866675, 1857429, 1820648]:
            print self.start
            print int(self.start)
            if self.start:
                cmds.currentTime(int(self.start), edit=True)
            ...
            for i in pm.ls(type="VRaySettingsNode"):
                if i.hasAttr("srdml"):
                    ...
                    if "vrayformaya" in self.plugins:
                        print type(self.plugins['vrayformaya'])
                        if self.plugins['vrayformaya'] >= "3.10":
                            print "Your task vray version >= 3.10,set srdml to 0 "
                            i.srdml.set(0)
            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")
        if self.user_id in [1862390]:
            for i in pm.ls(type="aiOptions"):
                # print "defaultArnoldRenderOptions.abortOnError 0"
                # i.abortOnError.set(1)
                if i.hasAttr("log_verbosity"):
        ...
        if self.user_id in [1860934]:
            ...
            if self.start:
                cmds.currentTime(int(self.start), edit=True)
                self.startFrame = self.start
                endFrame = self.start
                pm.PyNode("defaultRenderGlobals").self.startFrame.set(self.startFrame)
                pm.PyNode("defaultRenderGlobals").endFrame.set(self.startFrame)
                print "current render frame is %s" % (self.startFrame)

                # if self.user_id in [1163411]:
        ...
        if self.user_id in [1865164, 1870188, 10220697]:
            if self.rendersetting:
                for i in pm.ls(type="VRaySettingsNode"):
                    if 'width' in self.rendersetting and i.hasAttr("width"):
                        i.width.set(self.rendersetting['width'])
        ...
        if self.user_id in [1831496]:
            for i in pm.ls(type="aiOptions"):
                ...
                i.procedural_searchpath.set(
                    r"N:\cg_custom_setup\network\arnold\htoa-1.11.1_r1692_houdini-15.0.393_windows\arnold\procedurals")
                i.shader_searchpath.set(
                    r"N:\cg_custom_setup\network\arnold\htoa-1.11.1_r1692_houdini-15.0.393_windows\arnold\self.plugins")
                # setAttr -type "string" defaultArnoldRenderOptions.procedural_searchpath ;
                # setAttr -type "string" defaultArnoldRenderOptions.shader_searchpath "B:\\custom_config\\1831496\\htoa-1.11.1_r1692_houdini-15.0.393\\arnold\\self.plugins";
                i.renderType.set(0)

        if self.user_id in [1870536]:
            print self.start
        ...
        if self.user_id in [1885276]:
            print self.start
            if self.start:
                cmds.currentTime(int(self.start), edit=True)
                # mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")

        if self.user_id in [1819361]:
            print self.start
            if self.start:
                cmds.currentTime(int(self.start), edit=True)
            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")
        ...
        if self.user_id in [1888913]:
            for i in pm.ls(type="aiOptions"):
                if i.hasAttr("log_verbosity"):
                    i.log_verbosity.set(1)
                if i.hasAttr("autotx"):
                    i.autotx.set(False)
                if self.task_id in [10199980]:
                    if i.hasAttr("use_existing_tiled_textures"):
                        i.use_existing_tiled_textures.set(0)
                if i.hasAttr("textureMaxMemoryMB"):
                    i.textureMaxMemoryMB.set(20480)
        ...
        if self.user_id in [1898543]:
            mel.eval("optionVar -iv \"evaluationMode\" 1;")
            print "set evaluationMode to dg"
            mel.eval("putenv(\"MAYA_DISABLE_BATCH_RUNUP\",\"1\"); global proc dynRunupForBatchRender() {}; ")
        if self.user_id in [1844854]:
            print "Set rendering thread self.start>>>"
            for i in pm.ls(type="aiOptions"):
                if i.hasAttr("threads_autodetect"):
    ...
    def main(self):
        if self.user_id not in [1813119,962276,1820648,1849290]:
            self.get_scene_info()
