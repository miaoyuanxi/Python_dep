...
#            self["texture"] += [RvPath(i) for i in self.find_cache_files(i[0], i[1])]
                        ...
                        for attr in attrs:
                            ...
                            if r3:
                                for rr in r3:
                                    ...
                                    if rr:
                                        xgen_path.append(rr)
            xgen_path = [i.rstrip("/") for i in set(xgen_path)]
    ...
    def get_redshift(self):
        ...
        rs_proxy = [RvPath(i.fileName.get().strip())
                    ...
                    if i.fileName.get() if i.fileName.get().strip()]
        print "get RedshiftProxyMesh dir "
        ...
        for i in texture2:
            if i["exists"]==0:
                ...
                if not p.findall(i["client"]):
                    texture2 += [RvPath(i) for i in  self.file_images(os.path.join(pm.workspace(q=1, fullName=1), i["client" ]), 1)]
            ...
            if i.hasAttr("outputRSFolder"):
                if i.outputRSFolder.get():
                    if i.outputRSName.get():
                        texture.add(RvPath(os.path.join(i.outputRSFolder.get(),
                                                        i.outputRSName.get())))
            if i.hasAttr("cacheFolder"):
                if i.cacheFolder.get():
                    if i.cacheName.get():
                        texture.add(RvPath(os.path.join(i.cacheFolder.get(),
                                                        i.cacheName.get())))
    ...
    def file_images(self, udim_str, amin_attr=0):
        ...
        if os.path.exists(dir_path) == 1:
            ...
            file_name = file_name_src.replace('(', "\(").replace(")", "\)")
            # \%[\d]+[d]   "%05d"
