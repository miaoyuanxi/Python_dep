...
def start_maya(options):
    ...
    if int(options.user_id) == 1818936 or int(options.father_id) == 1818936:
        ...
        #     f.write(cmd_bat)
        # cmd = bat_tmp
    if int(options.user_id) in [1812148, 1830380, 1860293]:
    ...
    def __init__(self, path, exists=0):
        ...
        if self.is_win:
            ...
            if isinstance(self["client"], unicode):
                self["client"] = self["client"].encode("gb18030")

            if isinstance(self["server"], unicode):
                self["server"] = self["server"].encode("gb18030")
...
#            server_config["mounts"][project_drive] = "/" + project_drive.rstrip(":")
                    ...
                    for i in self["mappings"]:
                        ...
                        if not re.findall(r'^[a-z0-9 _.:()]+$', i, re.I):
                            ...
                            if not isinstance(i, unicode):
                                try:
                                    new_i = i.decode(sys.getfilesystemencoding())
                                except:
                                    try:
    ...
    def encode(self, string, code="gb18030"):
            ...
            if self.is_win:
                for i in self["mappings"].keys():
                    if isinstance(i, unicode):
                        self["mappings"][i.encode("gb18030")] = self["mappings"].pop(i)
    ...
    #            self["server"]])
                ...
                for i in self["mappings"]:
                    if RvOs.str_to_unicode(self["client"]).encode("gb18030").startswith(i):
                        path = RvOs.str_to_unicode(self["client"]).encode("gb18030").replace(i,
                            self["mappings"][i])
                        break
        ...
        with open(os.path.join(self.cfg_path, "render.cfg"), "w") as cfg_file:
            ...
            for step in ["texture", "zp"]:
                ...
                for index, path in enumerate(self[step]):
                    ...
                    if not path["exists"]:
                        self.missings.append(path["source"].encode("gb18030"))
                    ...
                    for path in self[step]:
                        ...
                        if path["exists"]:
                            ...
                            if self.options["platform"] in ["1007"]:
                                cfg_file.write("path%d=%s>>%s\r\n" % (index,
                                    path["client"], "".join([self.options["project_home"],
                                        path["server"]])))
                            else:
                                cfg_file.write("path%d=%s>>%s\r\n" % (index,
                                    path["client"], "".join([self.options["user_home"],
                            ...
                            self.missings.append(RvOs.str_to_unicode(path["source"]).encode("gb18030"))
                else:
                    if step == "texture":
                        if self.scene_compress:
...
#                raise Exception("Found reference file in the local machine.")
    ...
    def find_cache_files(self, cache_path, cache_name):
        return [os.path.join(cache_path, i)
                for i in os.listdir(cache_path)
                if i.lower().startswith(cache_name.lower())]
...
#            self["texture"] += [RvPath(i) for i in self.find_cache_files(i[0], i[1])]
        ...
        cache_paths = set([i.cachePath.get() for i in pm.ls(type="cacheFile")])
    ...
    def get_xgen(self):
        xgen = [i.xgFileName.get() for i in pm.ls(type="xgmPalette")
                if i.xgFileName.get()]
        xgen_format = []
        files1 = []
        files2 = []
        ...
        for i in xgen:
            ...
            if xgen_i["exists"]:
                ...
                xgen_abc = RvPath(xgen_i["client"].lower().replace(".xgen",
                                                                   ".abc"))
                if xgen_abc["exists"]:
        ...
        if xgen:
            ...
            file_str2 = [RvPath(i) for i in file_str2 if not i.startswith("$")]
            ...
            files1 = [RvPath(i) for i in files]
            ...
            data_path2 = [RvPath(i) for i in folders]
        ...
        files3 = [RvPath(i) for i in files3]

        print "get xgen files ok."
        result = xgen_format + files1 + files2 + files3 + files4 + data_path + data_path2 + file_str2
        abc_folder2 = []
        for i in result:
    ...
    def get_yeti(self):
        ...
        texture += [os.path.dirname(i.outputCacheFileName.get())
                    for i in yeti_node
                    if i if i.strip() if i.outputCacheFileName.get()]

        texture += [os.path.dirname(i.cacheFileName.get())
                    for i in yeti_node
                    if i if i.strip() if i.cacheFileName.get()]

        texture += [i.groomFileName.get()
                    for i in yeti_node
                    if i.hasAttr("groomFileName")
                    if i.groomFileName.get()]
        ...
        return [RvPath(i) for i in set(texture2) if i if i.strip()]
    ...
    def get_ass(self):
        ...
        ass += [RvPath(os.path.join(os.path.dirname(i), j))
                for i in all_ass
                if "#" in i
        ...
        texture += ass
        print ass
        print "get  ### ass files: " + str(len(ass))
    ...
    def get_ass_pixomon(self):
        ...
        all_ass = set([i.dso.get() for i in pm.ls(type='mesh') if i.hasAttr('dso') if i.ai_translator.get() == "procedural"])
    ...
    def get_redshift(self):
        texture = []
        texture += [RvPath(i.tex0.get().strip())
                    for i in pm.ls(type="RedshiftDomeLight")
                    if i.tex0.get() if i.tex0.get().strip()]
        texture += [RvPath(i.tex1.get().strip())
                    for i in pm.ls(type="RedshiftDomeLight")
                    if i.tex1.get() if i.tex1.get().strip()]
        rs_proxy = [RvPath(i.fileName.get().strip())
                    for i in pm.ls(type="RedshiftProxyMesh")
                    if i.fileName.get() if i.fileName.get().strip()]
        texture += [RvPath(os.path.dirname(i.fileName.get().strip()))
                    for i in pm.ls(type="RedshiftProxyMesh")
                    if i.useFrameExtension.get()
                    if i.fileName.get() if i.fileName.get().strip()]
        texture += [RvPath(i.tex0.get().strip())
                    for i in pm.ls(type="RedshiftSprite")
                    if i.tex0.get() if i.tex0.get().strip()]
        texture += [RvPath(i.tex0.get().strip())
                    for i in pm.ls(type="RedshiftNormalMap")
                    if i.tex0.get() if i.tex0.get().strip()]
        texture += [RvPath(i.tex0.get().strip())
                    for i in pm.ls(type="RedshiftEnvironment")
                    if i.tex0.get() if i.tex0.get().strip()]
        texture += [RvPath(os.path.dirname(i.fileName.get().strip()))
                    for i in pm.ls(type="RedshiftVolumeShape")
                    if i.fileName.get() if i.fileName.get().strip()]
        ...
        texture += [RvPath(i.dofBokehImage.get().strip())
                    ...
                    if i.hasAttr("dofBokehImage")
                    if i.dofBokehImage.get() if i.dofBokehImage.get().strip()]
        texture += [RvPath(os.path.dirname(i.irradianceCacheFilename.get().strip()))
                    for i in pm.ls(type="RedshiftOptions")
                    if i.irradianceCacheFilename.get() if i.irradianceCacheFilename.get().strip()]
        texture += [RvPath(os.path.dirname(i.irradiancePointCloudFilename.get().strip()))
                    for i in pm.ls(type="RedshiftOptions")
        ...
        #             for i in pm.ls(type="RedshiftVolumeShape")
        #             if i.fileName.get() if i.fileName.get().strip()
        #             if "<f>" not in i.fileName.get().strip()]
    ...
    def get_realflow(self):
        texture = [RvPath(i.path.get().strip())
                   for i in pm.ls(type="SDMeshModifier")
                   if i.path.get() if i.path.get().strip()]
    ...
    def get_miarmy(self):
        ...
        if pm.ls(type="McdMeshDriveIM"):
            ...
            for i in pm.ls(type="McdGlobal"):
                if os.path.exists(i.arProc.get()):
            ...
            if i.hasAttr("outputVRFolder"):
                if i.outputVRFolder.get():
                    if i.outputVRName.get():
                        texture.add(RvPath(os.path.join(i.outputVRFolder.get(),
                                                        i.outputVRName.get())))
    ...
    def get_vray(self):
        ...
        texture += [RvPath(i.fileName.get().strip())
                    for i in pm.ls(type="VRayTexOSL")
                    if i.fileName.get() if i.fileName.get().strip()]
        ...
        for i in pm.ls(type="VRaySettingsNode"):
            for i_attr in vray_settings_attr:
                if i.hasAttr(i_attr):
                    if i.attr(i_attr).get():
                        if i.attr(i_attr).get().strip():
                            texture.append(RvPath(os.path.dirname(i.attr(i_attr).get().strip())))
    ...
    def get_texture(self):
        node_types = ["mentalrayIblShape", "psdFileTex", "imagePlane",
                      "mentalrayTexture", "aiStandIn", "VRayMesh",
                      "RealflowMesh", "aiImage", "aiPhotometricLight",
        ...
        attr_names = ["imageName", "filename",
                      ...
                      "Paths[0]", "pmap_file2", "lc_fileName",
                      "cacheName", "cfp"]
...
#                    tex_attrs[node_i.attr(attr_i)] = node_i.attr(attr_i).get()

    #    all_types = set([i.type() for i in pm.ls()])

        textures = []
        for node_i in nodes:
            for attr_i in attr_names:
                if node_i.hasAttr(attr_i):
                    # print "Analyze Node: %s, Attr: %s" % (node_i, attr_i)
                    if node_i.hasAttr("useFrameExtension") \
                        and node_i.useFrameExtension.get() == 1:
                        # print node_i, attr_i
                        # try:
                        #    #ignore some chinese character error in some case
                        #    file = node_i.attr(attr_i).get()
                        # except:
                        #    file = None

                        file = node_i.attr(attr_i).get()
                        if file:
                            path = RvPath(os.path.dirname(file))
                            if os.path.exists(path["client"]):
    #                            textures += [os.path.join(path["client"], i)
    #                                for i in os.listdir(path["client"])
    #                                if i.decode("gb18030").startswith(os.path.basename(file).split(".")[0])]
                        ...
                        if file:
                            if "#" in file:
                                path = RvPath(os.path.dirname(file))
                                if os.path.exists(path["client"]):
                                    textures.append(path["client"])
                            elif file.lower().endswith(".bin"):
                                path = RvPath(os.path.dirname(file))
                                if os.path.exists(path["client"]):
                                    textures.append(path["client"])
                            elif re.findall(r'<udim>', file, re.I):
                                path = RvPath(os.path.dirname(file))
                                if os.path.exists(path["client"]):
                                    textures.append(path["client"])
                            elif re.findall(r'MAPID', file, re.I):
                                path = RvPath(os.path.dirname(file))
                                if os.path.exists(path["client"]):
                                    textures.append(path["client"])
                            elif re.findall(r'<UVTILE>', file, re.I):
                                path = RvPath(os.path.dirname(file))
                                if os.path.exists(path["client"]):
                                    textures.append(path["client"])
                            elif re.findall(r'<tile>', file, re.I):
                                path = RvPath(os.path.dirname(file))
                                if os.path.exists(path["client"]):
                                    textures.append(path["client"])
                            elif node_i.hasAttr("uvTilingMode"):
                                if node_i.uvTilingMode.get():
                                    path = RvPath(os.path.dirname(file))
                                    if os.path.exists(path["client"]):
                                        textures.append(path["client"])
                                else:
        ...
        print 0
        self.textures = []
        # with open("c:/rayvision.log.txt", "w") as f:
        #     for i in textures:
        #         f.write(i+"\n")
        ...
        for i in set(textures):
            if i:
                if i.strip():
                    ...
                    # else:
                    #     print i.decode("ascii", "replace").encode("ascii", "replace")
        ...
        if int(server_config["user_id"]) not in [100001]:
        ...
        self.textures = [i for i in set(self.textures)
                         if ".mayaSwatches" not in i["client"]]
        print 10

    def get_maya_files(self):
        textures = []

        if int(server_config["user_id"]) in [100001, 1813119]:
            for node_i in pm.ls(type="file"):
                file_path = node_i.fileTextureName.get()
                if file_path:
                    if file_path.strip():
                        textures += self.get_udim_files(file_path.strip())
        else:
            node_types = ["file"]
            ...
            for i in set(textures):
                if i:
                    if i.strip():
                        ...
                        #     print i.decode("ascii", "replace").encode("ascii", "replace")
                        if RvPath(i.strip())["exists"]:
                            texture2.append(RvPath(i.strip()))
                        elif "." in os.path.basename(RvPath(i.strip())["client"]):
                            texture2.append(RvPath(i.strip()))
    ...
    def get_udim_files(self, udim_path):
        ...
        result = glob.glob('%s.*%s' % (udim_path["client"].split('.')[0],
                                       udim_path["client"].split('.')[-1]))
        ...
        for i in ass_nodes.filter("procedural"):
            if "G_assetsPath" in i:
                if eval(i["G_assetsPath"][0]):
                    texture.append(RvPath(eval(i["G_assetsPath"][0])), in_ass=1)
                    break
    ...
    def get_nodes(self):
        ...
        for line in open_obj:
            ...
            if line:
                if line.startswith("#"):
                    pass
                # elif "program con not be run in DOS mode" in line:
                #     print line
                #     break
                    ...
                    if self.current_node:
                        ...
                        if len(line) == 1:
                            ...
                            for index, i in enumerate(line):
                                if index % 2:
                                    if index + 1 < len(line):
                                        self.current_node[line[index]] = [line[index + 1]]
...
#    print a.server_long_path
    ...
    a = u"我的gb18030".encode("gb18030")
    b = u"我的utf-8".encode("utf-8")
    c = u"我的unicode"
...
#
    ...
    ass = AssFile(r'C:\Users\admin\Desktop\aaa\fangzi04.ass')
