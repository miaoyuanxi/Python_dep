        ...
        if os.path.exists(api_json_path):
            ...
            for rendering_copy_interface in rendering_copy_interface_list:
                ...
                try:
                    ...
                    self.G_DEBUG_LOG.info("[response] {}".format(response.read().decode("utf-8")))
                    success = True
                    break
                    # print("模拟发请求 通知平台")
                    # time.sleep(1)
    ...
    def loop_handle_complete(self, render_list):
        ...
        while True:
            ...
            if count == frame_count:
                break
        if count != frame_count:
            raise Exception("渲染过程异常!!!")

    def remain_frames(self):
        """
        如果是重提并且存在一个json文件(有且只有一个):
            计算未完成的有哪些
            如果已经全部渲染完成:
                使用入口脚本的 frames
            否则:
        ...
        if int(self.G_RECOMMIT_FLAG) > 0 and os.path.isfile(json_path):
